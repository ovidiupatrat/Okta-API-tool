...
# A global variable to track the current theme state for toggling
is_light = True
...
# Update both the main progress bar and the circular Meter
...
def insert_transfer_object_with_children(parent, obj):
    """
    Insert an object into the Transfer TreeView under the given parent.
    If the object (a dict) has nested child data (e.g. under "child_resources"),
    these are recursively inserted as child nodes.
    """
    label = get_label_for_object(obj)
    node = transfer_tree.insert(parent, "end", text=label, values=("No",), open=True)
    transfer_mapping[node] = obj

    # If the object has a "child_resources" key, insert those as children.
    if isinstance(obj, dict) and "child_resources" in obj:
        ...
        if isinstance(child_dict, dict):
            for child_key, child_list in child_dict.items():
                # Create a node for this child resource category.
                child_parent = transfer_tree.insert(node, "end", text=child_key, values=("No",), open=True)
                transfer_mapping[child_parent] = None
                if isinstance(child_list, list):
                    for child_obj in child_list:
                        insert_transfer_object_with_children(child_parent, child_obj)
    return node

def insert_transfer_items(parent, items):
    """
    Recursively insert items (either a list or a dict) into the Transfer TreeView.
    """
    if isinstance(items, list):
        for item in items:
            if isinstance(item, dict):
                insert_transfer_object_with_children(parent, item)
            else:
                child = transfer_tree.insert(parent, "end", text=str(item), values=("No",), open=True)
                transfer_mapping[child] = item
    elif isinstance(items, dict):
        for key, value in items.items():
            sub_parent = transfer_tree.insert(parent, "end", text=key, values=("No",), open=True)
            transfer_mapping[sub_parent] = None
            insert_transfer_items(sub_parent, value)
...
def load_selected_snapshot_to_transfer():
    ...
    try:
        ...
        # Clear the transfer tree and mapping
        transfer_data.clear()
        for item in transfer_tree.get_children():
            transfer_tree.delete(item)
        transfer_mapping.clear()

        # Load the snapshot JSON (expected structure: {"tenant_url":..., "timestamp":..., "data": ...})
        with open(filename, "r", encoding="utf-8") as f:
            snapshot_json = json.load(f)
        loaded_data = snapshot_json.get("data", {})
        transfer_data.update(loaded_data)

        # Create a root node for the transfer snapshot
        root_node = transfer_tree.insert("", "end", text="Loaded Snapshot Transfer", values=("No",), open=True)
        transfer_mapping[root_node] = None

        # Process each top-level section (e.g., "Directory", "Customizations", etc.)
        for section_name, section_content in transfer_data.items():
            section_node = transfer_tree.insert(root_node, "end", text=section_name, values=("No",), open=True)
            transfer_mapping[section_node] = None
            if isinstance(section_content, dict):
                # Each key here is a resource (e.g. "Users", "Groups", etc.)
                for resource_name, objects_list in section_content.items():
                    resource_node = transfer_tree.insert(section_node, "end", text=resource_name, values=("No",), open=True)
                    transfer_mapping[resource_node] = None
                    if isinstance(objects_list, list):
                        for obj in objects_list:
                            insert_transfer_object_with_children(resource_node, obj)
                    else:
                        log_console(f"Warning: {resource_name} is not a list in snapshot.")
            elif isinstance(section_content, list):
                # If a section is directly a list, insert each object
                for obj in section_content:
                    insert_transfer_object_with_children(section_node, obj)
            else:
                log_console(f"Warning: Section {section_name} is neither a dict nor a list.")
...
def insert_object_with_children(parent, obj):
    """
    Insert a single object into the TreeView under 'parent'. If the object
    has nested child resources (under the "child_resources" key), recursively
    insert them as child nodes.
    """
    label = get_label_for_object(obj)
    node = overview_tree.insert(parent, "end", text=label, open=False)
    overview_mapping[node] = obj
    # Check if there are nested child resources in this object
    if isinstance(obj, dict) and "child_resources" in obj:
        child_dict = obj["child_resources"]
        if isinstance(child_dict, dict):
            for child_key, child_list in child_dict.items():
                # Insert a node for this category of child resources
                child_parent = overview_tree.insert(node, "end", text=child_key, open=True)
                overview_mapping[child_parent] = None
                if isinstance(child_list, list):
                    for child_obj in child_list:
                        insert_object_with_children(child_parent, child_obj)
    return node
...
def load_selected_snapshot_to_overview():
    ...
    try:
        ...
        # 'data' is what was saved under snapshot["data"], containing the same dict shape as overview_data
        loaded_data = snapshot_json.get("data", {})
        overview_data.update(loaded_data)

        # Create a root node for clarity
        root_node = overview_tree.insert("", "end", text="Loaded Snapshot Overview", open=True)
        overview_mapping[root_node] = None

        # For each top-level section (e.g., "Directory", "Applications", etc.)
        for section_name, section_content in overview_data.items():
            ...
            # Each section is expected to be a dict mapping resource names to lists of objects
            if isinstance(section_content, dict):
                for resource_name, objects_list in section_content.items():
                    resource_node = overview_tree.insert(section_node, "end", text=resource_name, open=True)
                    overview_mapping[resource_node] = None

                    if isinstance(objects_list, list):
                        for obj in objects_list:
                            # Use the recursive function to insert the object and any child resources
                            insert_object_with_children(resource_node, obj)
                    else:
                        log_console(f"Warning: {resource_name} is not a list in snapshot.")
            else:
...
def retrieve_overview():
    ...
    global_meter.configure(amounttotal=total_resources)
    global_progress_bar['value'] = 0
    global_meter.configure(amountused=0)
    global_progress_label["text"] = "Fetching tenant objects..."

    def process_child_resources(parent_node, parent_obj, resources, resource_name, section, res_name, okta_url, headers):
        """
        Process child resources for a given parent object.
        
        For each child resource definition in 'resources':
          - It formats the endpoint URL using the parent's id.
          - It fetches the child data using api_get().
          - If data is returned, it inserts the child objects into the TreeView.
          - It then updates the corresponding parent object (found in overview_data[section][res_name])
            by adding the child data under a new key "child_resources", keyed by the child resource's name.
        """
        for child_res in resources:
            # Build the endpoint URL (using parent's id, and brandId if available)
            child_endpoint = child_res["endpoint"].format(id=parent_obj["id"], brandId=parent_obj.get("id"))
            try:
                ...
                if child_resp.ok:
                    ...
                    if child_data:
                        # Insert child data into the TreeView
                        ...
                        for item in child_data:
                            item_id = overview_tree.insert(child_parent, "end",
                                                           text=get_label_for_object(item), open=False)
                            overview_mapping[item_id] = item
                        # Update the corresponding parent object in overview_data
                        if section in overview_data and res_name in overview_data[section]:
                            for parent_item in overview_data[section][res_name]:
                                if parent_item.get("id") == parent_obj.get("id"):
                                    parent_item.setdefault("child_resources", {})[child_res["name"]] = child_data
                                    break
                else:
                    log_console(f"Failed retrieving {child_res['name']} for {resource_name}: {child_resp.status_code}")
            except Exception as e:
    ...
    for section, resources in sections.items():
        ...
        for res in resources:
                ...
                if res_name == "Policies" and "policy_types" in res:
                                ...
                                for policy in policies:
                                    ...
                                    if "child_resources" in res:
                                        process_child_resources(policy_node, policy,
                                                                res["child_resources"], res_name, section, res_name, okta_url, headers)
                else:
                    ...
                    if response.ok:
                        ...
                        for obj in data:
                            ...
                            if "child_resources" in res:
                                process_child_resources(obj_node, obj, res["child_resources"], res_name, section, res_name, okta_url, headers)
                    else:
            ...
            fetched_count += 1
            smooth_progress_update(fetched_count)
...
def get_all_selected_items():
    """Recursively traverse the transfer tree and return a list of item IDs that are selected (value == 'Yes')
    and have an associated object in transfer_mapping."""
    selected_items = []
    def traverse(item):
        values = transfer_tree.item(item, "values")
        # If the node has an associated object (i.e. transfer_mapping is not None)
        # and is marked "Yes", add it.
        if values and values[0] == "Yes" and transfer_mapping.get(item):
            selected_items.append(item)
        # Recursively check its children.
        for child in transfer_tree.get_children(item):
            traverse(child)
    # Start from every top-level item.
    for item in transfer_tree.get_children():
        traverse(item)
...
def transfer_objects():
    ...
    # Use the recursive function to get all selected items.
    selected_item_ids = get_all_selected_items()
    total_items = len(selected_item_ids)
    global_progress_bar['maximum'] = total_items if total_items > 0 else 1
    global_meter.configure(amounttotal=total_items if total_items > 0 else 1)
    global_progress_bar['value'] = 0
    global_meter.configure(amountused=0)
    ...
    # Define the transfer order (this may determine the endpoint to use)
    transfer_order = [
        "okta_groups",
        "okta_users",
        "okta_idps",
    ...
    # We'll loop over each selected item.
    transferred = 0
    errors = 0
    processed = 0
    for item_id in selected_item_ids:
        ...
        obj = transfer_mapping.get(item_id)
        if not obj:
            continue

        # Determine the category of the object from its parent's text.
        parent_id = transfer_tree.parent(item_id)
        category = transfer_tree.item(parent_id, "text").lower()  # e.g., "users" or "groups"
        # Map the category (if needed) to the expected transfer key:
        if "user" in category:
            cat_key = "okta_users"
            ...
            # For other types you may want to add additional mapping logic.
            cat_key = category

        endpoint_suffix = transfer_endpoint_map.get(cat_key)
        if not endpoint_suffix:
            log_console(f"Skipping object {get_label_for_object(obj)}: No transfer endpoint for category {cat_key}.")
            continue

        full_endpoint = f"{target_url}{endpoint_suffix}"
        # Clean the object as needed
        ...
        try:
            ...
            while retries < max_retries:
                ...
                if resp.status_code == 429:
                    ...
                    log_console(
                        f"Rate limited during transfer. Waiting {sleep_time} seconds before retry..."
                    )
                    time.sleep(sleep_time)
                    wait_time *= 2
                    retries += 1
        ...
        processed += 1
        smooth_progress_update(processed)
...
def on_transfer_tree_select(event):
    """(Optional) Callback for selection change in Transfer tree."""
...
# Example DateEntry widget for filtering snapshots
...
# -- Move the circular progress meter to the right of the snapshots
...
# Global Progress Bar Frame (keeps the horizontal progress bar)
...
# Tree: has "Selected" column to indicate toggling
